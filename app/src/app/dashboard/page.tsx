...
import { Users, Wallet, RefreshCw, Clock, CheckCircle, AlertTriangle } from "lucide-react";
import { EncryptedBadge } from "@/components/EncryptedBadge";
import { StatCard } from "@/components/StatCard";
import { MOCK_VAULT, MOCK_USER, MOCK_REBALANCE_LOG } from "@/lib/mock-data";
  ...
  return (
    <div className="mx-auto max-w-7xl px-6 py-12 animate-fade-in">
      <div className="mb-10 flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-black text-white tracking-tight">Vault Dashboard</h1>
          <p className="mt-1 text-sm text-zinc-500">
            All balances and strategies are encrypted — only you can decrypt your share.
          </p>
        </div>
        <div className="flex items-center gap-2 rounded-xl border border-surface-border bg-surface-card px-4 py-2 text-xs">
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        {/* ── Chain allocation ──────────────────────────────────── */}
        <div className="lg:col-span-2 rounded-2xl border border-surface-border bg-surface-card overflow-hidden">
          <div className="border-b border-surface-border px-6 py-4 flex items-center justify-between">
            ...
            <span className="text-xs text-zinc-600">Targets encrypted on-chain · Display only</span>
          </div>
          ...
          <div className="border-t border-surface-border px-6 py-3 flex items-center gap-2 text-xs text-zinc-600">
            <AlertTriangle className="h-3.5 w-3.5 text-zinc-700" />
            Actual weights are encrypted FHE ciphertexts. Bars show configured targets only.
          </div>
        </div>
          ...
          <div className="p-6 space-y-4">
            {connected ? (
              <>
                ...
                <div className="rounded-lg border border-surface-border p-3 text-xs text-zinc-500 leading-relaxed">
      ...
      {/* ── Rebalance log ─────────────────────────────────────────── */}
      <div className="mt-6 rounded-2xl border border-surface-border bg-surface-card overflow-hidden">
        <div className="border-b border-surface-border px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-2">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-xs">
            <thead>
              <tr className="border-b border-surface-border text-zinc-600 uppercase tracking-wider">
            ...
            <tbody>
              {MOCK_REBALANCE_LOG.map((r) => (
                <tr key={r.id} className="border-b border-surface-border/50 hover:bg-surface-muted/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className="font-mono text-zinc-400">{shortAddress(r.solanaTx)}</span>
                  </td>
