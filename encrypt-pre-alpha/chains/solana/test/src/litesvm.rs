...
//! LiteSVM-based test context for multi-instruction integration testing.
//!
//! Provides `EncryptTestContext` — an ergonomic wrapper over `EncryptTestHarness<LiteSvmRuntime>`
//! with type-safe helpers for the full FHE lifecycle.
...
use crate::runtime_litesvm::LiteSvmRuntime;

/// Ergonomic test context for Encrypt-integrated Solana programs.
pub struct EncryptTestContext {
    harness: EncryptTestHarness<LiteSvmRuntime>,
}

impl EncryptTestContext {
    pub fn new(program_elf_path: &str) -> Self {
        let config = EncryptTestConfig {
            program_elf_path: program_elf_path.to_string(),
        };
        let runtime = LiteSvmRuntime::new();
    ...
    pub fn new_default() -> Self {
        let config = EncryptTestConfig::default_paths();
        let runtime = LiteSvmRuntime::new();
        let harness = EncryptTestHarness::new(runtime, config)
            .expect("failed to create test harness");
        Self { harness }
    ...
    pub fn create_input<T: EncryptedType>(
        &mut self,
        value: u128,
        authorized: &Pubkey,
    ) -> Pubkey {
    ...
    pub fn harness(&self) -> &EncryptTestHarness<LiteSvmRuntime> {
        &self.harness
    }

    pub fn harness_mut(&mut self) -> &mut EncryptTestHarness<LiteSvmRuntime> {
        &mut self.harness
    }
    ...
    // ── CPI / e2e test helpers ──

    pub fn deploy_program(&mut self, elf_path: &str) -> Pubkey {
        let program_id = Pubkey::new_unique();
        self.harness
            .tx_builder_mut()
            .runtime_mut()
            .deploy_program(&program_id, elf_path)
            .expect("deploy_program failed");
        program_id
    }

    pub fn deploy_program_at(&mut self, program_id: &Pubkey, elf_path: &str) {
        self.harness
            .tx_builder_mut()
            .runtime_mut()
            .deploy_program(program_id, elf_path)
    ...
    pub fn latest_blockhash(&self) -> Hash {
        self.harness.tx_builder().runtime().latest_blockhash()
    }
    ...
    pub fn register_ciphertext(&mut self, pubkey: &Pubkey) {
        ...
        self.harness.store().put(pubkey.to_bytes(), digest, fhe_type, None);
    }
    ...
    pub fn set_account(
        ...
        self.harness
            .tx_builder_mut()
            .runtime_mut()
            .set_account(pubkey, data, owner, lamports)
