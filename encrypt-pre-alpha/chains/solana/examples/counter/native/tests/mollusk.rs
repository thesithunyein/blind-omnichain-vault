...
//! Mollusk instruction-level tests for the confidential counter native example.
//!
...
const PROGRAM_PATH: &str = concat!(
    ...
    "/../../../../../target/deploy/confidential_counter_native"
);

// Native counter layout:
// disc(1) | authority(32) | counter_id(32) | value(32) | pending_digest(32) | revealed_value(8) | bump(1)
// Total: 138 bytes
const COUNTER_LEN: usize = 138;
...
fn build_counter_data(
    ...
    let mut data = vec![0u8; COUNTER_LEN];
