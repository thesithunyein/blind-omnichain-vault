...
//! LiteSVM end-to-end tests for the confidential counter anchor example.
//!
...
const EXAMPLE_PROGRAM_PATH: &str = concat!(
    ...
    "/../../../../../target/deploy/confidential_counter_anchor.so"
);
...
fn anchor_disc(name: &str) -> [u8; 8] {
    match name {
        ...
        "request_value_decryption" => [227, 24, 133, 5, 137, 80, 3, 80],
        "reveal_value" => [183, 128, 71, 133, 188, 49, 57, 213],
...
/// Build create_counter instruction with Anchor encoding.
/// Anchor ix data: disc(8) + counter_id(32) + initial_value_id(32)
/// Accounts: counter(init PDA), authority(signer), payer(signer,mut), system_program
fn create_counter_ix(
    program_id: &Pubkey,
    counter_pda: &Pubkey,
    counter_id: &[u8; 32],
    initial_value_id: &Pubkey,
    authority: &Pubkey,
    payer: &Pubkey,
) -> Instruction {
...
/// Anchor ix data: disc(8) + cpi_authority_bump(1)
fn increment_ix(
    program_id: &Pubkey,
    counter: &Pubkey,
    value_ct: &Pubkey,
...
fn test_create_counter() {
    ...
    // Anchor layout: 8-byte disc + Counter fields
    // authority(32) counter_id(32) value(32) pending_digest(32) revealed_value(8) bump(1)
    let data = ctx.get_account_data(&counter_pda).expect("counter not found");
    assert!(data.len() >= 8 + 32, "should have anchor disc + data");
    assert_eq!(&data[8..40], authority.pubkey().as_ref()); // authority
    assert_eq!(&data[40..72], &counter_id); // counter_id
    assert_eq!(&data[72..104], value_ct.as_ref()); // value
}
...
fn test_increment() {
    ...
    // Create ciphertext for value via harness
