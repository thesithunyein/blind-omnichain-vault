...
//! Mollusk instruction-level tests for the encrypted ACL native example.
//!
...
/// Build Resource account data (native layout, no anchor discriminator).
...
/// Build AccessCheck account data (native layout).
fn build_access_check_data(
    ...
    let mut data = vec![0u8; ACCESS_CHECK_LEN];
