...
//! LiteSVM end-to-end tests for the encrypted ACL native example.
//!
//! Tests the full lifecycle: create_resource -> grant -> revoke -> check -> decrypt.
...
const EXAMPLE_PROGRAM_PATH: &str = concat!(
    ...
    "/../../../../../target/deploy/encrypted_acl_native.so"
);

/// Deploy the ACL program and return (program_id, cpi_authority, cpi_bump).
fn setup_acl_program(ctx: &mut EncryptTestContext) -> (Pubkey, Pubkey, u8) {
...
/// Build a create_resource instruction.
/// data: disc(1=0) | resource_bump(1) | cpi_authority_bump(1) | resource_id(32)
fn create_resource_ix(
    ...
    data.push(0u8); // discriminator: create_resource
...
/// Build a grant_permission instruction.
/// data: disc(1=1) | cpi_authority_bump(1)
...
/// Build a revoke_permission instruction.
/// data: disc(1=2) | cpi_authority_bump(1)
...
/// Build a check_permission instruction.
/// data: disc(1=3) | check_bump(1) | cpi_authority_bump(1)
fn check_permission_ix(
    ...
    Instruction::new_with_bytes(
        *program_id,
        &[3u8, check_bump, cpi_authority_bump],
        vec![
...
fn create_resource(
    ...
    (resource_pda, perm_pubkey, resource_id)
}
...
fn do_check(
    ...
    ctx.send_transaction(&[ix], &[checker, &result_ct]);

    let result_pubkey = result_ct.pubkey();
    ctx.register_ciphertext(&result_pubkey);

    let graph = check_permission_graph();
    ctx.enqueue_graph_execution(&graph, &[*permissions_ct, bit_ct], &[result_pubkey]);
    ctx.process_pending();
    ctx.register_ciphertext(&result_pubkey);
...
fn test_create_resource() {
    ...
    let (program_id, cpi_authority, cpi_bump) = setup_acl_program(&mut ctx);
    let admin = ctx.new_funded_keypair();

    let (resource_pda, _perm_ct, _rid) =
        create_resource(&mut ctx, &program_id, &cpi_authority, cpi_bump, &admin);

    let data = ctx.get_account_data(&resource_pda).expect("resource not found");
    assert_eq!(data[0], 1); // discriminator = RESOURCE
}
