#!/usr/bin/env bun
/**
 * Confidential Voting E2E Demo — gill
 *
 ...
 *   Terminal 3: just demo-gill <ENCRYPT_ID> <VOTING_ID>
 */
...
if (!encryptArg || !votingArg) {
  console.error(
    "Usage: bun e2e-voting-gill.ts <ENCRYPT_PROGRAM_ID> <VOTING_PROGRAM_ID>"
  );
  process.exit(1);
}
...
const { rpc, rpcSubscriptions, sendAndConfirmTransaction } =
  createSolanaClient({ urlOrMoniker: RPC_URL });
...
/**
 * Build and send a transaction using gill's createSolanaClient helpers.
 */
...
async function main() {
  console.log("\n\x1b[1m\u2550\u2550\u2550 Confidential Voting E2E \u2014 gill \u2550\u2550\u2550\x1b[0m\n");
  ...
  const airdropSig = await rpc.requestAirdrop(payer.address, 100_000_000_000n as any).send();
  for (let i = 0; i < 30; i++) {
  ...
  const encVaultBytes = configData.slice(100, 132);
  ...
  for (const { name, vote } of votes) {
    ...
    await rpc.requestAirdrop(voter.address, 1_000_000_000n).send();
  ...
  await sendTx(payer, [
    {
      programAddress: VOTING_PROGRAM,
      data: Uint8Array.from([3, cpiBump, 1]),
      accounts: [
        acct(proposalPda, AccountRole.WRITABLE),
  ...
  await sendTx(payer, [
    {
      programAddress: VOTING_PROGRAM,
      data: Uint8Array.from([3, cpiBump, 0]),
      accounts: [
        acct(proposalPda, AccountRole.WRITABLE),
  ...
  await pollUntil(yesReq.address, (d) => {
  ...
  await pollUntil(noReq.address, (d) => {
    if (d.length < 107) return false;
    const total = readU32LE(d, 99);
    const written = readU32LE(d, 103);
    return written === total && total > 0;
