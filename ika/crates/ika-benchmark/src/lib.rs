...
use ika_json_rpc_types::{
    IkaObjectDataOptions, IkaObjectResponse, IkaObjectResponseQuery, IkaTransactionBlockEffects,
    IkaTransactionBlockEffectsAPI, IkaTransactionBlockResponseOptions,
};
use ika_sdk::{IkaClient, IkaClientBuilder};
use ika_types::effects::{TransactionEffectsAPI, TransactionEvents};
...
use ika_types::{base_types::ObjectRef, crypto::AuthorityStrongQuorumSignInfo, object::Owner};
...
pub mod util;
pub mod workloads;
use ika_types::quorum_driver_types::{QuorumDriverError, QuorumDriverResponse};
...
pub enum ExecutionEffects {
    ...
    IkaTransactionBlockEffects(IkaTransactionBlockEffects),
}
...
impl ExecutionEffects {
    pub fn mutated(&self) -> Vec<(ObjectRef, Owner)> {
        match self {
            ...
            ExecutionEffects::IkaTransactionBlockEffects(ika_tx_effects) => ika_tx_effects
                .mutated()
                .iter()
    ...
    pub fn quorum_sig(&self) -> Option<&AuthorityStrongQuorumSignInfo> {
        match self {
            ...
            ExecutionEffects::IkaTransactionBlockEffects(_) => None,
        }
    }
    ...
    pub fn gas_object(&self) -> (ObjectRef, Owner) {
        match self {
            ...
            ExecutionEffects::IkaTransactionBlockEffects(ika_tx_effects) => {
                let refe = &ika_tx_effects.gas_object();
                (refe.reference.to_object_ref(), refe.owner)
            }
        }
    }

    pub fn sender(&self) -> IkaAddress {
    ...
    pub fn gas_cost_summary(&self) -> GasCostSummary {
        match self {
            ...
            crate::ExecutionEffects::IkaTransactionBlockEffects(b) => {
...
pub trait ValidatorProxy {
    ...
    async fn get_validators(&self) -> Result<Vec<IkaAddress>, anyhow::Error>;
}
    ...
    async fn execute_transaction_block(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        while retry_cnt < 3 {
            let ticket = self
                .qd
                .submit_transaction(
                    ika_types::quorum_driver_types::ExecuteTransactionRequestV3 {
...
impl FullNodeProxy {
    pub async fn from_url(http_url: &str) -> Result<Self, anyhow::Error> {
        ...
        let resp = ika_client.read_api().get_committee_info(None).await?;
...
impl ValidatorProxy for FullNodeProxy {
    async fn get_object(&self, object_id: ObjectID) -> Result<Object, anyhow::Error> {
        let response = self
            .ika_client
            .read_api()
            .get_object_with_options(object_id, IkaObjectDataOptions::bcs_lossless())
            .await?;

        if let Some(ika_object) = response.data {
            ika_object.try_into()
            ...
            let response = self
                ...
                .get_owned_objects(
                    ...
                    Some(IkaObjectResponseQuery::new_with_options(
                        IkaObjectDataOptions::bcs_lossless(),
                    )),
                    cursor,
                    None,
                )
    ...
    async fn execute_transaction_block(&self, tx: Transaction) -> anyhow::Result<ExecutionEffects> {
        ...
        while retry_cnt < 10 {
            ...
            {
                Ok(resp) => {
                    return Ok(ExecutionEffects::IkaTransactionBlockEffects(
    ...
    fn clone_new(&self) -> Box<dyn ValidatorProxy + Send + Sync> {
        Box::new(Self {
            ika_client: self.ika_client.clone(),
